...
int s_socket_get_peer_name(ssocket_t *socket_s)
{
	...
	return getpeername(socket_s->socket, (struct sockaddr *)&socket_s->address, (socklen_t *)&socket_s->address_len);
...
int s_socket_get_sock_name(ssocket_t *socket_s)
{
	...
	return getsockname(socket_s->socket, (struct sockaddr *)&socket_s->address, (socklen_t *)&socket_s->address_len);
...
{
	size_t len;
	len = sendto(socket_s->socket,message, length, flags,(const struct sockaddr *)&info_s->address, info_s->address_len);
	if(len==-1)
	{
		socket_s->nb_error_sent++;
		return -1;
...
{
	size_t len;
	len = recvfrom(socket_s->socket,buffer, length,flags,(struct sockaddr *) &info_s->address,(socklen_t *)  &info_s->address_len);
	if(len==-1)
	{
		socket_s->nb_error_recv++;
		return -1;
...
{
	int ret;
	if ((ret=accept(socket_s->socket, (struct sockaddr *)&info_s->address, (socklen_t *)&info_s->address_len)) < 0) 
		return -1;
...
{
	return shutdown(socket_s->socket,how);
}

int s_socket_set_sock_opt(ssocket_t *socket_s, int level, int option_name,const void *option_value, int option_len)
{
	return setsockopt(socket_s->socket, level, option_name,option_value, option_len);
...
int s_socket_get_sock_opt(ssocket_t *socket_s, int level, int option_name,void *option_value, int *option_len)
{
	...
	return getsockopt(socket_s->socket, level, option_name,option_value, (socklen_t *)  option_len); 
